class TagWriter {
	...
	connectTag() {
    	window.mifare.connect(() => ons.notification.toast('NFC Tag Connected', {timeout: 500}), err => ons.notification.toast(`Couldn't connect because: ${err}`, {timeout: 3000}));                		
	}
	...
	WriteBaseTag() {
		...
		ons.notification.toast('Writing Basic Ultimaker Tag Schema', {timeout: 500})
		...
		$("#tag_write_progress").val('25.0');
		// when tag has been written, inform user
		ons.notification.toast('Basic Tag Schema Written. Do not disconnect tag...', {timeout: 500})
	}
	/*
	...
	WriteGUIDToTag(materialName, materialGUID) {
		...
		window.mifare.write(20, address20.split(" "), (response) => console.log(response), err => ons.notification.toast('Writing failed at address ' + i + ' ' + err, {timeout: 500}));
		$("#tag_write_progress").val('45.0');

		console.log('started to read')
		window.mifare.read(16, (response) => Object.assign(readOutput, {16: response.data.substr(0,8)}), err => console.log(`Reading failed because ${err}`));
		...
		window.mifare.read(20, (response) => Object.assign(readOutput, {20: response.data.substr(0,8)}), err => console.log(`Reading failed because ${err}`));
		...
		setTimeout(function() {
			...
			setTimeout(function() {
				ons.notification.alert("GUID for <strong>"+materialName+"</strong> <br>has been written to tag");
